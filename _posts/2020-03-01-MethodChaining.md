...
ReactiveX라던가 Alamofire 같은 라이브러리 코드를 보면 코드를 점(.)으로 이어가면서 한번에 쭉 기술하는 것을 볼 수 있습니다. 바로 체이닝을 사용한 것인데요. 라이브러리 뿐만 아니라 Swift에서도 기본적으로 사용할 수 있는 체이닝들이 있습니다.

- **예제 코드**

    ```swift
    let string = "aaa|bbb|ccc|aaa|bbb|ccc|aaa|bbb|ccc|aaa|bbb|ccc|aaa|bbb|ccc|"
    
    let count = string.split(separator: "|").filter { $0 == "aaa" }.count
    print(count) /** 5 **/
    ```

    이 코드는 문자열을 split한 후 filter를 통해 걸러진 데이터의 개수를 구하는 방법 입니다. 체이닝으로 연결이 안 된다면 split, filter, count를 각각 따로 입력 받아서 처리해야 되지만 체이닝을 통해 한 줄로 구현이 되었습니다.
...
- **코드 분석**
    ...
    - split
        ...
        ```swift
        func split(separator: Character, maxSplits: Int = Int.max, omittingEmptySubsequences: Bool = true)
        -> [Substring]
        ```

        split 메서드를 보면 반환 타입이 **[Substring]**으로 되어 있습니다. 우리는 이것을 통해 문자열을 split 하면 배열이 나온다는 것을 유추할 수 있고, 배열에서 사용 가능한 메서드를 체이닝 할 수 있다는 것을 알게 됩니다.
...
위에서는 이미 Swift에서 지원하고 있는 함수 체이닝을 보았습니다.  근데 체이닝 방식은 지원해주는 것만 쓸 수 있을까요? 당연히 아닙니다. 우리에겐 Extension이라는 아주 매력적인 것이 있죠. Extension을 통해 우리가 만들고 싶은 체이닝을 만들어서 사용할 수 있습니다.

- **UIBezierPath로 삼각형 그리기**

    체이닝으로 제가 많이 활용하는 것이 UIBezierPath입니다. 간단하게 UIBezierPath로 삼각형을 그려 보도록 하겠습니다.
    ...
    - 기존 방식으로 그린 삼각형
        ...
        path.move(to: center)
        path.addLine(to: CGPoint(x: center.x - 100, y: center.y + 100))
        path.addLine(to: CGPoint(x: center.x + 100, y: center.y + 100))
    ...
    - 체이닝을 위한 extension 만들기
        ...
        extension UIBezierPath {
            func move(_ point: CGPoint) -> Self {
                move(to: point)
                return self
            }
        
            func addLine(_ point: CGPoint) -> Self {
                addLine(to: point)
                return self
            }
        }
        ...
        이제 UIBezierPath를 extension하여 기존에 반환 타입이 Void인 move와 addLine에 **Self**를 반환하는 메서드를 만들어 주었습니다. 
    ...
    - 체이닝을 이용해 그린 삼각형
        ...
        let path = UIBezierPath()
                    .move(center)
                    .addLine(CGPoint(x: center.x - 100, y: center.y + 100))
                    .addLine(CGPoint(x: center.x + 100, y: center.y + 100))
                    .addLine(center)
        ...
        체이닝을 이용해서 path를 만들어 주었습니다. move, addLine을 점(.)으로 연결해서 계속 작성해 나가는 것을 볼 수 있습니다.
...
- **CAShapeLayer 설정도 체이닝으로 변경하기**
    ...
    - 체이닝을 위한 extension 만들기
        ...
        extension CAShapeLayer {
            func setPath(_ path: UIBezierPath) -> Self {
                self.path = path.cgPath
                return self
            }
        
            func setStrokeColoe(_ color: UIColor) -> Self {
                self.strokeColor = color.cgColor
                return self
            }
        
            func setStrokeEnd(_ value: CGFloat) -> Self {
                self.strokeEnd = value
                return self
            }
        
            func setLineWidth(_ width: CGFloat) -> Self {
    ...
    - 체이닝을 이용해 설정한 CAShapeLayer
        ...
        체이닝을 이용해 설정한 CAShapeLayer입니다. 기존 방식보다 좀 더 직관적으로 보입니다.
