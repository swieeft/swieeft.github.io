...
- ViewController 간 연관 관계를 한 눈에 볼 수 있음
- ViewController의 형태를 시각적으로 표현해줘서 UI가 어떻게 생겼는지 확인할 수 있음
- 컨트롤 생성 시 Drag & Drop으로 간단히 생성 가능

하지만 프로젝트가 점점 커지고, 협업하는 인원이 많아지게 되면 Storyboard는 많은 문제점을 발생시키게 되는데 대표적으로 발생하는 2가지의 문제점은

- Git Marge 시 Storyboard에서 충돌이 발생함
- Storyboard에 여러 개의 ViewController가 추가되면 Storyboard 실행이 느려지고, 심지어 Xcode가 멈춰 버리는 일도 발생함

이러한 문제를 해결하기 위해 2가지 방법을 많이 사용하게 되는데요. 

- Storyboard를 사용하지 않고 100% 코드로 UI 개발
- One Storyboard, One ViewController(하나의 스토리보드에 하나의 뷰컨트롤러만) = Storyboard 나누기

이렇게 2가지 방법을 많이 사용하는데, 이 글에서는 2번째 방법인 Storyboard를 나눌 때 필요한 Storyboard Reference 사용 방법에 대한 내용을 정리하려고 합니다.

> 프로젝트를 새로 생성하면 Main Storyboard와 ViewController가 생성되어 있는데요. 이 두 개를 프로젝트 메인화면으로 사용해서 진행하도록 하겠습니다.

### 기본 방법으로 Segue로 연결하기

---

우선 Storyboard Reference 사용하는 방법을 알아보기 전에 기본적으로 ViewController간의 Segue 연결 방식 및 동작을 보도록 하겠습니다.
...
2.  **FirstViewController를 First Storyboard로 이동하기**
    ...
    새로 생성하는 경우에는 First Storyboard에서 ViewController를 바로 생성해주시면 됩니다.
...
4. **Initial View Controller 설정해주기**
    ...
    Storyboard Reference를 사용해서 이동하게 되는 ViewController는 해당 스토리보드의 진입 ViewController가 되어야 합니다.

    진입 ViewController로 설정하기 위해선 Attributes Inspector에서 is Initial View Controller를 체크 해주면 됩니다.
